...
Network.prototype.load = function (callback) {
    ...
    utils.parallel(Object.keys(this.blogs), 1, function (blog_name, next) {
        ...
        for (var name in self.blogs) {
            self.blogs[name].post_array.forEach(function (post) {
                post.blog = name;
            });
            self.post_array = self.post_array.concat(self.blogs[name].post_array);
        }
...
Network.prototype.keys = function (key) {
    ...
    for (var name in this.blogs) {
        this.blogs[name].keys(key).forEach(function (key) {
            keys[key] = 1;
        });
    }
