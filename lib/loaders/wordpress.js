...
WordpressLoader.prototype.load = function (callback) {
    ...
    var select_posts = format('SELECT ID, post_title, post_content, post_status, post_type, ' +
            'post_date_gmt, post_modified_gmt, guid FROM %s_posts', this.prefix)
    ...
    this.db.query(select_options, function (err, rows) {
        ...
        blog = utils.merge({}, self.metadata, blog);
        ...
        self.db.query(select_posts, function (err, rows) {
            ...
            self.db.query(select_postmeta, function (err, rows) {
                ...
                self.db.query(select_terms, function (err, rows) {
                    ...
                    self.normalise(blog, posts, function (err, blog, posts) {
                        if (err) {
                            return callback(err);
                        }
                        try {
                            self.push(posts, blog);
                        } catch (e) {
                            return callback(e);
                        }
                        callback();
...
WordpressLoader.prototype.normalise = function (blog, posts, callback) {
    ...
    for (var id in posts) {
        ...
        if (post.title && post.date && post.status === 'publish' && post.type === 'post') {
            normalised_posts[id] = post;
        }
    }
    callback(null, blog, normalised_posts);
};
