...
Network.prototype.posts = function (query, limit, offset, callback) {
    ...
    for (var i = 0, len = this.post_array.length, count = 0; i < len; i++) {
        if (utis.query(this.post_array[i], query)) {
...
Blog.prototype.post = function (blog_name, slug, callback) {
 ...
 *
 * @param {Function} fn - receives (blog_name, blog)
 */
