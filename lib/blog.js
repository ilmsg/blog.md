...
Blog.prototype.select = function (options) {
    ...
    if (options.fill && posts.length < options.limit) {
        var seen_ids = posts.map(function (post) {
            return post.id;
        });
        var query = { id: { $nin: seen_ids }};
        if (options.query && typeof options.query.id === 'object' && options.query.id.$nin) {
            query.id.$nin.concat(options.query.id.$nin);
        }
