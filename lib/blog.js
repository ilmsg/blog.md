...
Blog.prototype.index = function (posts) {
    ...
    posts.forEach(function (post) {
        ...
        if (!slug) {
            ...
            while (slug in self.post_slugs) {
                slug = (slug_title + '-' + (i++)).trim('-');
            }
            post.slug = slug;
        }
        self.post_slugs[slug] = 0;
        post.next = null;
        post.prev = null;
    });
    ...
    posts.forEach(function (post, index) {
        self.post_slugs[post.slug] = index;
...
Blog.prototype.post = function (slug, callback) {
    var self = this;
    var post = null;
    if (slug in self.post_slugs) {
        post = self.post_array[self.post_slugs[slug]];
    }
    callback(null, post);
};
...
Blog.prototype.posts = function (query, limit, offset, callback) {
    ...
    for (var i = 0, post, match, len = this.post_array.length, count = 0; i < len; i++) {
        post = this.post_array[i];
        if (!query) {
            match = true;
...
Blog.prototype.count = function (query) {
    ...
    for (var i = 0, post, match, len = this.post_array.length; i < len; i++) {
        post = this.post_array[i];
        if (!query) {
            match = true;
        } else if (typeof post.match === 'function') {
