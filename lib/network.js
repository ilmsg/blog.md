...
Network.prototype.posts = function (query, limit, offset, callback) {
    ...
    utils.parallel(selected, 10, function (post, next) {
        self.fetchPost(post.blog_name, post.slug, function (err, post) {
            if (err) {
                return next(err);
            }
        ...
        callback(null, selected.map(function (post) {
            return complete[post.blog_name + ':' + post.slug];
        }));
    });
...
Network.prototype.fetchPost = function (blog_name, slug, callback) {
    if (!(blog_name in this.blogs)) {
        return callback('Unknown blog ' + blog_name);
    }
    this.blogs[blog_name].fetchPost(slug, callback);
};
