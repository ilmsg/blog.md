...
Blog.prototype.load = function (callback) {
    ...
    this.loader.load(function (err, blog) {
        ...
        //Verify all posts have the required keys
        if (self.options.required_keys) {
            var keys = self.options.required_keys.length;
            for (var i = 0, len = self.posts.length; i < len; i++) {
                for (var j = 0, key; j < keys; j++) {
                    key = self.options.required_keys[j];
                    if (!(key in self.posts[i])) {
                        return callback(format('Post does not contain "%s" (%s)',
                            key, self.posts[i].id));
                    }
                }
            }
        ...
        //Sort posts by date descending if there's no custom sorter
